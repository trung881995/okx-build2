...
var ASM_CONSTS = {
  2994464: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 2994525: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 2994589: function() {return Module.webglContextAttributes.powerPreference;},  
 2994647: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 2994702: function($0) {performance.now = function() { return $0; };},  
 2994750: function($0) {performance.now = function() { return $0; };},  
 2994798: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  async function _connectOKXWallet() {
          let suiData = [76, 111, 103, 105, 110, 32, 119, 105, 116, 104, 32, 66, 108, 117, 101, 109, 111, 118, 101];
          let uint8Array = new Uint8Array(suiData);
          
  if (!window.okxTonConnectUI) {
                  console.error("OKX Wallet is not initialized.");
                  return;
              }
          
              await window.okxTonConnectUI.openModal();
          
     
            
      }

  function _disconnectOKXWallet() {
        window.okxTonConnectUI.disconnect();  
      }
  ...
  function _getAccountInfo() {
        const currentAccount = window.okxTonConnectUI.account;
        
        const jsonString = JSON.stringify(currentAccount);
        window.unityInstance.SendMessage('OKXManager', 'OnGetCurrentAccount', jsonString);  
      }
  ...
  function _getWalletConnected() {
       const isConnected    = window.okxTonConnectUI.connected;
       if(isConnected)
       {
           window.unityInstance.SendMessage('OKXManager', 'OnCheckWalletConnected', "Connected"); 
           return;
       }
         window.unityInstance.SendMessage('OKXManager', 'OnCheckWalletConnected', "Disconnected"); 
      }

  function _getWalletInfo() {
        const currentWallet  = window.okxTonConnectUI.wallet;
        
        const jsonString = JSON.stringify(currentWallet);
        window.unityInstance.SendMessage('OKXManager', 'OnGetCurrentWallet', jsonString);  
      }
...
var asmLibraryArg = {
  ...
  "abort": _abort,
  "connectOKXWallet": _connectOKXWallet,
  "disconnectOKXWallet": _disconnectOKXWallet,
